...
| --- | --- |
| **Export to Toolbag** or **Create Bake Project** | Blender writes the file and **launches** Toolbag with the REVO Bridge plugin as a boot script. The plugin window opens and Toolbag imports automatically |
...
The plugin opens automatically only when Blender launches Toolbag. For an
existing Toolbag session, open it once from **Edit > Plugins > Revo_Bridge**.
...
USD transfers selected meshes, UVs, normals, vertex colors, material assignments, USD Preview Surface materials and referenced textures. FBX is the compatibility option.
...
**Import from Toolbag** in Blender loads a USD Toolbag already wrote. It does not start the Toolbag plugin. If the REVO Bridge window is not open, open **Edit > Plugins > Revo_Bridge**, click **Export Scene to Blender**, then import (or wait for Blender auto-import).
